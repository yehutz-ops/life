...
export default function QuickCaptureBar() {
  const { items, projects, addInboxEntry, addItem } = useStore()
  ...
  const matches = useMemo(() => {
    const q = text.trim()
    if (!q || aiResult || aiLoading) return []
    return items.filter((it) => it.title.includes(q)).slice(0, 5)
  }, [text, items, aiResult, aiLoading])
          ...
          {aiResult.intent === 'create_draft' && aiResult.draft && (
            <div>
              ...
              <div className="text-xs text-stone-500 dark:text-stone-400 space-y-0.5 mb-3">
                ...
                {aiResult.draft.relatedPerson && <div>אדם: {aiResult.draft.relatedPerson}</div>}
                <div>רמת ביטחון: {aiResult.confidence === 'high' ? 'גבוהה' : aiResult.confidence === 'medium' ? 'בינונית' : 'נמוכה'}</div>
              </div>
